...
author: "Cat Phan, Athena Kwok, Christina Dang, Ye Chan Yoo, Nithya Shamini Chandran"
output:
...
- [paste0 function](https://stackoverflow.com/questions/28028110/insert-character-at-end-of-string-in-r-except-for-the-last-element) This `paste0()` function works similar to `paste()` but the default `sep` is an empty string (`""`) in `paste0()` and the default `sep` is a single space (`" "`) in `paste()`.
...
xmas <- xmas %>%
  mutate(
    days_in_words = as.character(english(Day)),
